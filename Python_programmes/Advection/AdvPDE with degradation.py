...
# Time span
t_span = (0, 2500)           # Time range for simulation
dt = 0.02                    # Time step for numerical evaluation
t_steps = int(t_span[-1]/dt) 


# initial_values=[1, 1.5, 0.5]
# initial_values1=[0, 1, 2]
...
#Y0 = np.zeros([t_steps])
#Y0[0] = 0
print('tipo', type(Y), 'dimesiones', np.ndim(Y), 'forma', np.shape(Y))
...
# plt.subplot(2, 1, 1)
# for j in tqdm(range(0, len(mu_values))):
#     plt.plot(time, X[:, j], color=colors[j], label=f'PDE μ = {mu_values[j]}')
# plt.xlabel("time")
# plt.ylabel("X(t)")
...
plt.subplot(2, 1, 1)
# for j in tqdm(range(0, len(mu_values))):
#     plt.plot(time[-100000:], X[-100000:, j], label=f'μ = {mu_values[j]}')
