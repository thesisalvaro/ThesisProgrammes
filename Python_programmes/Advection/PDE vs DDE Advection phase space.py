...
# Initial conditions
X = np.ones([t_steps, len(m_values)])     # Initial value for X
...
#         Yprime = YDDE[int(i - 1 - tau/dtDDE), j] if i - tau/dtDDE > 0 else YDDE[0, j]
#         k1_x = X_equationDDE(XDDE[i-1, j], Yprime, m, mu, Delta, a)
#         k1_y = Y_equationDDE(XDDE[i-1, j], YDDE[i-1, j], k1, k2, K, m, q1, q2)
#         k1_y_prime = Y_equationDDE(XDDE[int(i-1 - tau/dtDDE), j], YDDE[int(i-1-tau/dtDDE), j], k1, k2, K, m, q1, q2) if i - tau/dtDDE > 0 else YDDE[0, j]

#         aux_x = XDDE[i-1, j]+dtDDE*k1_x
#         aux_y = YDDE[i-1, j]+dtDDE*k1_y
#         aux_y_prime = Yprime+dtDDE*k1_y_prime if i- tau/dtDDE > 0 else YDDE[0, j]
...
#         YDDE[i, j] = YDDE[i-1, j]+0.5*dtDDE*(k1_y+k2_y)
...
# amplitud_finalDDE[j] = amplitud_finalDDE[j] * k1 /(q1 * (K **m))
# periodo_finalDDE[j] = periodo_finalDDE[j] / q1

# chunk_size = 50000  # Example chunk size
# XDDE_reescaled = np.empty_like(XDDE) 
# YDDE_reescaled = np.empty_like(YDDE)
#  # Preallocate memory for the result

# for start in range(0, XDDE.shape[0], chunk_size):
#     end = start + chunk_size
#     XDDE_reescaled[start:end] = XDDE[start:end] * k1 / (q1 * (K ** m))

# for start in range(0, YDDE.shape[0], chunk_size):
#     end = start + chunk_size
#     YDDE_reescaled[start:end] = YDDE[start:end] * k1 / (q1 * (K ** m))

XDDE_reescaled = XDDE.astype(np.float32) * k1 /(q1 * (K **m))
YDDE_reescaled = YDDE.astype(np.float32) * K
timeDDE_reescaled = timeDDE / q1

# amplitud_finalDDE = amplitud_finalDDE * k1 /(q1 * (K **m))
# periodo_finalDDE = periodo_finalDDE / q1

##### 

## This is DDE with ddeint
