##PDE RK diffusion pero con downsample
...
# def Y_equation(t, x, y, k2, q2, D, dx, mu): 
#     y[0] = 0
#     y[1:499] = (D/(dx**2))*(-2*y[1:499]+y[0:498]+y[2:500]) - mu * y[1:499]
#     y[500] = k2*x-q2*y[500]
#     y[501:-2] = (D/(dx**2))*(-2*y[501:-2]+y[500:-3]+y[502:-1]) - mu * y[501:-2]
#     y[-1] = 0
#     return y
...
#prev
downsampled_steps = t_steps // downsample_factor  # Number of time steps in the downsampled array
...
#X = np.zeros((t_steps, 1))  # Adjust the number of species as needed
    ...
    for i in tqdm(range(1,t_steps)):
        ...
        if i % downsample_factor == 0:
            ...
            if downsample_index < downsampled_steps:  # Ensure within bounds
                ...
                #prev
            # X_downsampled[downsample_index] = X[i, j]
            # Y_downsampled[:, downsample_index] = Y_current
...
#estos son  los que uso de prueba
# plt.plot(time, X, label="X(t)", color = 'b')
# plt.plot(time_downsampled, Y_downsampled[delta_idx, :], label="Y_delta", color = 'orange', linestyle= 'dashed')
# plt.plot(time_downsampled, Y_downsampled[0, :], label="Y(t)", color='g')
